...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (await currentUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError(
        'User recently changed password. Try with new password',
        '401',
      ),
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('User with the provided email doesn\'t exist', '404'),
    );
  }
  ...
  const message = `Forgot your password? Change your password using the following link: ${resetURL} If you didn't forget your password, please ignore this email`;
  try {
    await sendEmail({
      email: user.email,
